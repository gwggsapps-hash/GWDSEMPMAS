...
function getTokenType(token) {
    if (parseToStandardDate(token))
        return DataType.DATE;
    if (/^\d{8,}$/.test(token))
        return DataType.NUMBER;
    if (/[a-zA-Z]/.test(token))
        return DataType.TEXT_ENGLISH;
    if (/^\d+(\.\d+)?$/.test(token))
        return DataType.NUMBER;
    return DataType.UNKNOWN;
}
function mergeRecords(rawRows) {
    if (rawRows.length === 0)
        return [];
    const merged = [];
...
export function harmonizeData(rawRows) {
    if (rawRows.length === 0)
    ...
    originalHeaders.forEach((header, index) => {
        const types = new Set();
        if (index === 0) {
            types.add(DataType.TEXT_ENGLISH);
        }
        else {
    ...
    originalHeaders.forEach((header, index) => {
        ...
        if (index === 0 || types.size <= 1) {
            ...
            const subs = [];
            if (types.has(DataType.TEXT_ENGLISH))
                subs.push(`${header} (Text)`);
            if (types.has(DataType.NUMBER))
                subs.push(`${header} (Number)`);
            if (types.has(DataType.DATE))
                subs.push(`${header} (Date)`);
        ...
        originalHeaders.forEach((header, index) => {
            ...
            if (index === 0 || targets.length === 1) {
                ...
                tokens.forEach(token => {
                    ...
                    else if (type === DataType.TEXT_ENGLISH) {
                        subSuffix = " (Text)";
                    }
...
export function extractDashboardMetrics(data) {
...
export function generatePivotData(rows, rowField, colField, valField, aggType) {
    const pivot = {};
    ...
    rows.forEach(row => {
        ...
        if (!pivot[rVal])
            pivot[rVal] = {};
        if (aggType === 'sum') {
            pivot[rVal][cVal] = (pivot[rVal][cVal] || 0) + vVal;
        }
        else {
            pivot[rVal][cVal] = (pivot[rVal][cVal] || 0) + 1;
        }
    });
